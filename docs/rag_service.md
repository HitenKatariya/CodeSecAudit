...
**Live endpoint**: https://hitenvk22-codesec-rag-service.hf.space
...
```bash
# Health
curl https://hitenvk22-codesec-rag-service.hf.space/health

# Search
curl -X POST https://hitenvk22-codesec-rag-service.hf.space/rag/search \
...
> **Status**: The RAG service is already live at https://hitenvk22-codesec-rag-service.hf.space.
...
export HF_RAG_SPACE_ID="hitenvk22/codesec-rag-service"
...
git init
git remote add origin https://huggingface.co/spaces/hitenvk22/codesec-rag-service
git add .
git commit -m "deploy CodeSecAudit RAG service"
git push origin main
```

Then add secrets manually at https://huggingface.co/spaces/hitenvk22/codesec-rag-service/settings:
| Secret | Required | Description |
|---|---|---|
| `RAG_API_KEY` | **Production** | Shared API key for request auth. If empty, public. |
| `RAG_DATASET_REPO` | No | HF dataset repo (default: `hitenvk22/CodeSecAudit-RAG-Shuffled`) |
...
```bash
export CODESEC_RAG_SERVICE_URL=https://hitenvk22-codesec-rag-service.hf.space
...
```
Checking remote RAG service at: https://hitenvk22-codesec-rag-service.hf.space
