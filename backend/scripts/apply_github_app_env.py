...
Usage:
    python backend/scripts/apply_github_app_env.py          # dry-run
    python backend/scripts/apply_github_app_env.py --write  # actually write
    python backend/scripts/apply_github_app_env.py --help
"""
import argparse
import json
import os
...
def load_credentials(path: Path) -> dict:
    if not path.exists():
        ...
        print("Run: python backend/scripts/complete_github_app_manifest.py --code YOUR_CODE", file=sys.stderr)
        sys.exit(1)
    with open(path) as f:
        return json.load(f)
