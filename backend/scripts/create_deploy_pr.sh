...
- \`python backend/scripts/prepare_hf_rag_space.py\`
- \`python -m compileall backend rag-service/rag_service website\`
- \`python backend/scripts/evaluate_reviewer.py\`
- \`python backend/scripts/review_code.py --code \"eval(user_input)\" --json\`
"
...
git add \
    rag-service/rag_service/index.py \
    rag-service/rag_service/main.py \
    rag-service/README.md \
    backend/scripts/prepare_hf_rag_space.py \
    backend/scripts/deploy_hf_rag_space.py \
    backend/scripts/check_remote_rag_service.py \
    backend/scripts/check_main_api_remote_rag.py \
    backend/scripts/create_deploy_pr.sh \
    backend/scripts/test_remote_rag_client.py \
    backend/scripts/test_rag_service.py \
    docs/rag_service.md \
    .gitignore \
    2>/dev/null || true
